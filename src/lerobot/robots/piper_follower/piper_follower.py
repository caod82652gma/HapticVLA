...
class PIPERFollower(Robot):
    ...
    def __init__(self, config: PIPERFollowerConfig):
        ...
        # Optional tactile sensor support.
        self.tactile_enabled = config.tactile_enabled
        self.tactile_sensor = (
            make_tactile_sensor_from_config(config.tactile) if self.tactile_enabled else None
        )
    ...
    def connect(self) -> None:
        ...
        if self.tactile_enabled and self.tactile_sensor:
            try:
                ...
                print(f"tactile sensor connected: {self._tactile_connected}")
            except Exception as e:
                logger.warning("Failed to connect tactile sensor: %s", e)
                logger.warning("Continue without tactile sensor")
                self._tactile_connected = False
    ...
    def disconnect(self) -> None:
        ...
        if self.tactile_sensor and self._tactile_connected:
            try:
                ...
                logger.warning("Error disconnecting tactile sensor: %s", e)
            self._tactile_connected = False
    ...
    def get_observation(self) -> dict:
        ...
        if self.tactile_enabled and self._tactile_connected and self.tactile_sensor:
            try:
                ...
                logger.warning("Failed to get tactile observation: %s", e)
