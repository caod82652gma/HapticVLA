...
def make_robot_from_config(config: RobotConfig) -> Robot:
    ...
    elif config.type == "mobile_base_client":
        from .mobile_base import MobileBaseClient
        
        return MobileBaseClient(config)
    elif config.type == "lekiwi":
        from .lekiwi import LeKiwi

        return LeKiwi(config)
    elif config.type == "crab":
        from .crab import Crab

        return Crab(config)
    elif config.type == "crab_client":
        from .crab import CrabClient

        return CrabClient(config)
