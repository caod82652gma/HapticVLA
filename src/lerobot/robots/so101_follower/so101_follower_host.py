...
class SO101FollowerServerConfig:
    """Configuration for the SO101Follower host script."""

    robot: SO101FollowerConfig = field(default_factory=SO101FollowerConfig)
    host: SO101FollowerHostConfig = field(default_factory=SO101FollowerHostConfig)
...
@draccus.wrap()
def main(cfg: SO101FollowerServerConfig):
    logging.info("Configuring SO101Follower")
    robot = SO101Follower(cfg.robot)

    logging.info("Connecting SO101Follower")
    robot.connect()

    logging.info("Starting HostAgent")
    host = SO101FollowerHost(cfg.host)
    ...
    try:
        ...
        print("Shutting down SO101Follower Host.")
        robot.disconnect()
        host.disconnect()

    logging.info("Finished SO101Follower cleanly")
