...
class SO101FollowerClient(Robot):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "SO101Follower Daemon is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for SO101Follower Host to connect expired.")
    ...
    def _get_data(self) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
        ...
        If successful, updates and returns the new frames and arm state.
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("SO101FollowerClient is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command arm to move to a target joint configuration. Sends over ZMQ
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "SO101Follower is not connected. You need to run `robot.connect()` before disconnecting."
            )
