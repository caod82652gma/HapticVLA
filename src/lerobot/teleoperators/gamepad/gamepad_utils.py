...
def apply_linear_deadzone(value: float, deadzone: float) -> float:
    """Apply deadzone with linear rescaling for smooth response.
    
    Instead of abruptly cutting off at deadzone (causing jumpy feel),
    this rescales the output so that:
    - Values within deadzone return 0
    - Values outside deadzone are linearly scaled from 0 to 1
    """
    if abs(value) < deadzone:
        return 0.0
    sign = 1.0 if value > 0 else -1.0
    # Rescale so deadzone maps to 0 and 1.0 maps to 1.0
...
class GamepadController(InputController):
    ...
    def start(self):
        ...
        logging.info(f"Found {joystick_count} joysticks.")
    ...
    def get_deltas(self):
        ...
        try:
            ...
            # Apply deadzone with linear rescaling for smooth response
            x_input = apply_linear_deadzone(x_input, self.deadzone)
            y_input = apply_linear_deadzone(y_input, self.deadzone)
            z_input = apply_linear_deadzone(z_input, self.deadzone)
    ...
    def _update(self):
        ...
        try:
            ...
            if data and len(data) >= 8:
                ...
                # Apply deadzone with linear rescaling for smooth response
                self.left_y = apply_linear_deadzone(self.left_y, self.deadzone)
                self.left_x = apply_linear_deadzone(self.left_x, self.deadzone)
                self.right_x = apply_linear_deadzone(self.right_x, self.deadzone)
                self.right_y = apply_linear_deadzone(self.right_y, self.deadzone)
